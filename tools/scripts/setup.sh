...
elif [ "$enable_node_discovery" == "n" ]; then
...
if [ "$environment_type" -eq 1 ]; then
    ...
    create_kind_clusters "$consumers_json" "$providers_json" $environment_type 1 1 $enable_node_discovery
    ...
    # Call create_kind clusters with parameters and save return value into clusters variable
    create_kind_clusters "$consumers_json" "$providers_json" $environment_type "$consumer_clusters" "$provider_clusters" $enable_node_discovery
...
install_components "$consumers_json" "$providers_json" $local_repositories $enable_auto_discovery $installation_type $enable_node_discovery
