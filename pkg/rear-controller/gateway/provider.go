...
func (g *Gateway) StartHttpServer() {
	...
	router.HandleFunc("/api/purchaseflavour/{flavourID}", g.purchaseFlavour).Methods("POST")
...
func (g *Gateway) getFlavoursBySelector(w http.ResponseWriter, r *http.Request) {
	...
	klog.Infof("Flavour selected is: %s - Parsing...", selected.Spec.FlavourID)
...
func (g *Gateway) purchaseFlavour(w http.ResponseWriter, r *http.Request) {
	...
	klog.Infof("Purchasing flavour %s started", purchase.FlavourID)
	...
	klog.Infof("Performing purchase of flavour %s...", purchase.FlavourID)

	// Remove the transaction from the transactions map
	g.removeTransaction(transaction.TransactionID)

	klog.Infof("Flavour %s purchased!", purchase.FlavourID)

	// Get the flavour sold for creating the contract
	flavourSold, err := services.GetFlavourByID(purchase.FlavourID, g.client)
	if err != nil {
